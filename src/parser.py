...
_EVENT_LABEL_RE = re.compile(
    r"(?:일정|날짜|행사일|행사\s*날짜|기간|개최일|이벤트\s*날짜"
    r"|Date|When|Schedule|Event\s*[Dd]ate|Registration)[\s:：]+",
    re.IGNORECASE,
)
...
def _find_date(text: str) -> str:
    ...
    이벤트 날짜를 추출한다.
    우선순위:
      1. 이벤트 레이블(일정:, Date: 등) 바로 뒤의 날짜
      2. 본문 내 모든 날짜 중 오늘 이후의 가장 가까운 미래 날짜
      3. 위 모두 없으면 "미정"
    """
    today = date.today()

    # 1단계: 이벤트 레이블 기반 우선 검색
    for label_m in _EVENT_LABEL_RE.finditer(text):
        snippet = text[label_m.end(): label_m.end() + 60]
        date_m = _DATE_RE.match(snippet.strip())
        if date_m:
            return date_m.group().strip()

    # 2단계: 본문의 모든 날짜 추출 → 미래 날짜 우선
    all_matches = [m.group().strip() for m in _DATE_RE.finditer(text)]
    if not all_matches:
        return "미정"

    future = []
    for raw in all_matches:
        d = _parse_date_to_obj(raw)
        if d and d >= today:
            future.append((d, raw))

    if future:
        future.sort(key=lambda x: x[0])
        return future[0][1]  # 가장 가까운 미래 날짜

    # 미래 날짜가 없으면 "미정" (모두 과거 → 만료 처리됨)
    return "미정"
...
def parse(page: PageResult) -> dict:
    ...
    is_expired = False
    if page.category_id != "online_course":
        d = _parse_date_to_obj(event_date)
        if d is not None:
...
def parse_all(pages: list[PageResult]) -> list[dict]:
    ...
    for page in pages:
        ...
        if item["is_expired"]:
            expired_count += 1
            print(f"  ⏭️  만료 제외: {item['title'][:50]} ({item['date']})")
            continue
        results.append(item)

    if expired_count:
        print(f"  → 만료된 일정 총 {expired_count}건 제외")
    return results
